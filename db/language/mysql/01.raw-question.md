...
1. acid
   - a: undo-log
   - c: [aid 实现]事务执行结束后，数据库的**完整性约束**没有被破坏
     1. **自定义完整性**: 一个事务在执性之前和执行之后, 数据库都必须处以一致性状态[如果从 A 账户转账到 B 账户, 不可能因为 A 账户扣了钱, 而 B 账户没有加钱] + 事务外看数据也是一一致的(s 数据库的完整性约束)[钱没扣成功课就不能少]
     2. 实体完整性: 如行的主键存在且唯一
     3. 列完整性: 如字段的类型、大小、长度要符合要求
     4. 外键约束
   - i: 并发执行的各个事务之间不能互相干扰, mvcc[undo-log] + 事务隔离级别
   - d: redo-log + 数据不丢失[崩溃重启]
2. tx_isolantion
