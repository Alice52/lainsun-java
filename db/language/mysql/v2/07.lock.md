...
3. 分类 2
   ...
   - 意向锁: 为了解决表锁和行锁冲突 , 意向共享锁 & 意向排他锁 || 表级锁, 不会和行级的 X, S 锁发生冲突, 只会和表级的 X, S 发生冲突
...
4. 常考如何锁定一行: innodb + ~~可重复读下{任何隔离级别下都可以}~~~
...
1. 死锁: 两个及以上线程互相持有对方需要的资源, 而形成的僵持就是死锁
...
2. 死锁: `innodb_deadlock_detect=on`
3. 避免死锁:
