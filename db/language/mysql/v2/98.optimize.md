...
12. 如果 order by 上不适合建立索引, 则一定要深度过滤: 在高并发下还是有问题
13. on 高于 where: 但是注意含义是不一样的
14. where 高于 having, 能写在 where 限定的条件就不要去 having 限定
15. **高并发场景下减少对唯一对唯一索引的使用: 尽量使用覆盖索引 + 业务逻辑判断**
16. 尽量不要按照两张表的两个字段 group: 数量级上来之后一定会变慢
17. ORDER BY 尽量避免复合升序降序: 建立索引时也要这么做
18. 考虑使用`读已提交`的事物隔离级别
19. 尽量索引轻量级的字段: 比如能索引 int 字段就不要索引 varchar 字段
20. 索引字段也可以是部分前缀, 在创建的时候指定字段索引长度
21. 针对长文本的搜索, 可以考虑使用 Elasticsearch 等专门用于文本搜索的索引数据库
22. 真的需要的话: 可以考虑使用外键
23. 创建索引
...
24. 合理的增加冗余字段
25. 所有字段都要有默认值: null 的影响
26. force index() 防止优化器选错索引
27. 尽量不要使用关联子查询: 过程就慢
28. **适当增加冗余字段**: 不是所有的东西都能解决的{有时候需要业务的妥协}
29. 最小数据长度 + 最简单数据类型: 不要把字段定义的很大
30. 如果查询包括 GROUP BY 但你并不想对分组的值进行排序, 你可以指定 ORDER BY NULL 禁止排
31. in/notin 转换为 join 快是因为不需要临时表(子查询)
