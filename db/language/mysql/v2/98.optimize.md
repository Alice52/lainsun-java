...
1. sql 执行慢的原因
   ...
   - 机器的原因: `cpu + io + config: top + free + iostat + vmstat`
...
11. extra
    ...
    - using temporary: 会有性能问题[及时速度很快, 还是有可能在并发下吃 cpu], 临时表保存中间结果; 常见于排序 order by 和分组查询 group by 字段和索引不一致
    - using index
      ```sql
...
2. 尽量每张表不要超过 5 个索引
   ...
   - 全值匹配
   - 最左前缀: 条件顺序
   - groupby 要和 orderby 一起: 执行时需要, 否则就可能用到临时表或文件排序
   - 索引失效{**覆盖索引时除去 is-null 不会索引失效**}: or/计算函数/范围[<>in!=like{`x%x%`}]/涉及类型转换/null{is null 索引一直会失效/is not null 在非覆盖索引才会失效}
